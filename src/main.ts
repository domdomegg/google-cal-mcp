...
(async () => {
	if (transport === 'stdio') {
		...
		if (!accessToken) {
			console.error('google-calendar-mcp: GOOGLE_ACCESS_TOKEN required for stdio transport');
		...
		if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
			console.error('google-calendar-mcp: GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET required for HTTP transport');
		...
		const oauthMetadata: OAuthMetadata = {
			...
			scopes_supported: CALENDAR_SCOPES,
		};
		...
		const protectedResourceMetadata: OAuthProtectedResourceMetadata = {
			...
			scopes_supported: CALENDAR_SCOPES,
			resource_name: 'Google Calendar MCP Server',
			resource_documentation: 'https://github.com/domdomegg/google-calendar-mcp',
		};
		...
		const httpServer = app.listen(port, () => {
			console.error(`Google Calendar MCP server running on ${baseUrl}/mcp`);
		});
